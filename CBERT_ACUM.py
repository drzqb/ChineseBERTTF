...
from FuncUtils import gelu, softmax, create_initializer, computePRF, load_model_weights_from_npy_cbert
...
PARAMS_model = "CBERT_ACUM_sighan"
...
class Mask(Layer):
    ...
    def call(self, senwrong, **kwargs):
        ...
        sum_ls = tf.reduce_sum(tf.cast(sequencemask, tf.float32))

        return mask, sequencemask, seq_length, sum_ls
...
class USER():
    ...
    def build_model(self, summary=True, print_fn=None):
        ...
        mask, sequencemask, seqlen, sum_ls = Mask(name="mask")(noise)
        ...
        TPD, TND, FPD, TPC, TNC, FPC, TPSD, TNSD, FPSD, TPSC, TNSC, FPSC, \
        PD, RD, FD, PC, RC, FC, PSD, RSD, FSD, PSC, RSC, FSC = AllMetrics(self.tokenizer.chardictlen, name="metrics")(
            inputs=(sen, noise, logitschar, sequencemask, sum_ls, newpri, newthresh, newthreshup))
        ...
        tf.keras.utils.plot_model(model, to_file="CBERT_ACUM.jpg", show_shapes=True, dpi=900)
    ...
    def train(self):
        ...
        if PARAMS_mode == 'train0':
            ...
            # load_model_weights_from_npy_cbert(model)
            model.load_weights("modelfiles/CBERT_ACUM_271k/CBERT_ACUM.h5")
            ...
            model.save(PARAMS_check + '/CBERT_ACUM.h5')

            with open(PARAMS_check + '/CBERT_ACUM.txt', 'w', encoding='utf-8') as fs:
                fs.write(str(number))

        else:
            model = tf.keras.models.load_model(PARAMS_check + '/CBERT_ACUM.h5',
                                               custom_objects={
            ...
            with open(PARAMS_check + '/CBERT_ACUM.txt', 'r', encoding='utf-8') as fs:
        ...
        for epoch in range(number, PARAMS_epochs + 1):
            ...
            with open(PARAMS_check + '/CBERT_ACUM.txt', 'w', encoding='utf-8') as fn:
