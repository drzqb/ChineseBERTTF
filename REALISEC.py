...
from tensorflow.keras.layers import Conv2D, BatchNormalization, Activation, add, Flatten
...
from FuncUtils import gelu, softmax, create_initializer, computePRF, checkpoint_loader
...
PARAMS_model_prefix = "REALISEC"
PARAMS_model = PARAMS_model_prefix + "_271k"
...
def load_model_weights_from_checkpoint_bert(model, checkpoint_file):
    ...
    w1 = model.get_layer('embeddings').get_weights()
    weights = [
    ...
    model.get_layer('encoderchar').set_weights(weights)
    ...
    weights = weights_a + weights_f
    model.get_layer('encoderpy').set_weights(weights)
    ...
    weights = weights_a + weights_f
    model.get_layer('fuse').set_weights(weights)
...
class BasicBlock(Layer):
    ...
    def build(self, input_shape):
        self.conv1 = Conv2D(self.filter_num, (3, 3), strides=2, padding='same')
        self.bn1 = BatchNormalization()
        self.relu = Activation('relu')
        self.conv2 = Conv2D(self.filter_num, (3, 3), strides=1, padding='same')
        self.bn2 = BatchNormalization()

        self.downsample = Conv2D(self.filter_num, (1, 1), strides=2)
...
class GLYPH(Layer):
    def __init__(self, **kwargs):
        super(GLYPH, self).__init__(**kwargs)

    def build(self, input_shape):
        self.block1 = BasicBlock(64, name="block1")
        self.block2 = BasicBlock(128, name="block2")
        self.block3 = BasicBlock(192, name="block3")
        self.block4 = BasicBlock(192, name="block4")
        self.flatten = Flatten(name="flatten")
        super(GLYPH, self).build(input_shape)

    def call(self, inputs, **kwargs):
        x, seqlen = inputs
        x = tf.reshape(x, [-1, PARAMS_font_size, PARAMS_font_size, PARAMS_font_num])
        x = self.block1(x)
        x = self.block2(x)
        x = self.block3(x)
        x = self.block4(x)
        x = self.flatten(x)
        x = tf.reshape(x, [-1, seqlen, PARAMS_hidden_size])
        return x
...
class Embeddings(Layer):
    ...
    def build(self, input_shape):
        ...
        font_array = np.stack(font_arrays, axis=3)
        self.glyphembeddings = Embedding(PARAMS_vocab_size,
                                         ...
                                         trainable=False)
        self.glyph_map = GLYPH(name="glyph_map")
    ...
    def call(self, inputs, **kwargs):
        ...
        glyph_embed = self.glyph_map((glyph_embed, seqlen))
        ...
        return self.layernormanddropsen((sen_embed + other_embed, rate)), \
               ...
               self.layernormanddropglyph((glyph_embed, rate)), \
...
def train_step(model, data, pri, rate, thresh, threshup):
...
class USER():
    ...
    def build_model(self, summary=True, print_fn=None):
        ...
        now = NewEmbeddings(name="gate")(inputs=(nowsen, nowpy, glyph_embed, sequencemask, sum_ls, seqlen, newrate))
        ...
        PD, RD, FD, PC, RC, FC, PSD, RSD, FSD, PSC, RSC, FSC = AllMetrics(name="metrics")(
            inputs=(
                sen, noise, logitschar, sequencemask, sum_ls_all, newpri, newthresh,
                newthreshup))
