...
from FuncUtils import gelu, create_initializer, computePRF
from TFDataUtils import single_example_parser, batched_data

PARAMS_bert_path = "pretrained/ChineseBERT-base"
...
PARAMS_train_file = [
    # 'data/tfrecordfile/bench_271k_train.tfrecord',
    'data/tfrecordfile/bench_sighan_train.tfrecord',
]
PARAMS_test_file = [
    # 'data/TFRecordFile/bench_sighan_train.tfrecord',
    # 'data/tfrecordfile/bench_sighan_test_wrong.tfrecord',
    'data/tfrecordfile/bench_sighan_test.tfrecord',
]

PARAMS_model = "CCBERT_sighan"
...
# def load_model_weights_from_npy_ccbert(model):
...
#
#     weights = model.get_layer("fuse").get_weights()
#     w1 = []
#     for i in range(12):
#         w1.extend(
...
#             [weights[t] for t in range(2 + 8 * i, 4 + 8 * i)] +
...
def load_model_weights_from_npy_ccbert(model):
    ...
    for i in range(12):
        w1.extend(
            ...
            [weights[t] for t in range(4 + 10 * i, 6 + 10 * i)] +
            [np.transpose(allweights["bert.encoder.layer." + str(i) + ".attention.output.dense.weight"]),
             allweights["bert.encoder.layer." + str(i) + ".attention.output.dense.bias"],
             np.transpose(allweights["bert.encoder.layer." + str(i) + ".attention.output.LayerNorm.weight"]),
             allweights["bert.encoder.layer." + str(i) + ".attention.output.LayerNorm.bias"]]
...
class Mask(Layer):
    ...
    def call(self, senwrong, **kwargs):
        ...
        return sequencemask, seq_length, sum_ls, sum_ls_all
...
#         self.conv1d = Conv1D(PARAMS_hidden_size, 3, padding="same")
...
#
#     def call(self, inputs, **kwargs):
#         x, sequence_mask, sumls, seqlen, rate = inputs
#         q = self.dense_q(x)
#         conv1d = self.conv1d(q)
#
...
#
#         conv1d_f = tf.where(sequence_mask_expand, conv1d, tf.zeros_like(conv1d))
#
#         conv1d_av = tf.reduce_sum(conv1d_f, axis=1) / sum_ls_expand
#         avpool = tf.tile(tf.expand_dims(conv1d_av, axis=1), [1, seqlen, 1])
#
#         attention_output = self.dense_o(conv1d + avpool)
#
...
#         avpool = tf.tile(tf.expand_dims(conv1d_av, axis=1), [1, seqlen, 1])
#
#         attention_output = self.dense_o(conv1d + tf.nn.sigmoid(tf.abs(conv1d - avpool)) * avpool)
#
#         return self.layernorm(x + tf.nn.dropout(attention_output, rate))
...
class Attention(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        attention_output = self.dense_o(conv1d)
...
class Encoder(Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        for k in range(self.layers):
            x = self.ffd[k](inputs=(self.attention[k](inputs=(x, sequence_mask, sumls, seqlen, rate)), rate))
...
def train_step(model, batch, data, pri, rate, thresh, threshup):
    ...
    gradients = tape.gradient(meanloss, trainable_variables)
...
class USER():
    ...
    def build_model(self, summary=True, print_fn=None):
        ...
        sequencemask, seqlen, sum_ls, sum_ls_all = Mask(name="mask")(noise)

        sen_embed, word_embeddings = Embeddings(name="embeddings")(inputs=(noise, noisepy, seqlen, newrate))
        now = Encoder(layers=12, name="fuse")(inputs=(sen_embed, sequencemask, sum_ls, seqlen, newrate))
        ...
        PD, RD, FD, PC, RC, FC, PSD, RSD, FSD, PSC, RSC, FSC = AllMetrics(name="metrics")(
            inputs=(sen, noise, logitschar, sequencemask, sum_ls_all, newpri, newthresh, newthreshup))
        ...
        tf.keras.utils.plot_model(model, to_file="CCBERT.jpg", show_shapes=True, dpi=900)
    ...
    def train(self):
        ...
        if PARAMS_mode == 'train0':
            ...
            load_model_weights_from_npy_ccbert(model)
            # model.load_weights("modelfiles/CCBERT_271k/CCBERT.h5")
            ...
            model.save(PARAMS_check + '/CCBERT.h5')

            with open(PARAMS_check + '/CCBERT.txt', 'w', encoding='utf-8') as fs:
                fs.write(str(number))

        else:
            model = tf.keras.models.load_model(PARAMS_check + '/CCBERT.h5',
                                               custom_objects={
            ...
            with open(PARAMS_check + '/CCBERT.txt', 'r', encoding='utf-8') as fs:
        ...
        for epoch in range(number, PARAMS_epochs + 1):
            ...
            for batch, tb in enumerate(train_batch):
                ...
                if (batch + 1) % 10 == 0 or batch + 1 == PARAMS_per_save:
                    ...
                    fw.write(
                        'Epoch %5d/%5d %5d/%5d [%s] -loss: %.6f -acc:%6.1f'
            ...
            with open(PARAMS_check + '/CCBERT.txt', 'w', encoding='utf-8') as fn:
